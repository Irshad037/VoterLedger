...
const CriminalCase = ({ criminalCases, setCriminalCases }) => {
    ...
    const removeCriminalCase = (id) => {
        setCriminalCases(criminalCases.filter(c => c.id !== id));
    };
            ...
            {criminalCases.map((c, index) => (
                ...
                >
                    ...
                    <input
                        ...
                        accept=".pdf"
                        onChange={(e) =>
                            updateCriminalCase(c.id, "file", e.target.files[0])
                        }
                    />

                    {c.file && (
                        <p className="text-xs text-green-600 mt-1">
                            {c.file.name} uploaded
                        </p>
                    )}
                </div>
            ))}
