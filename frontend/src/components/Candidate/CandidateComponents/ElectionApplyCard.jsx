...
const ElectionApplyCard = ({ election }) => {
    ...
    return (
        <div className={`group bg-white rounded-4xl border-2 transition-all duration-300 relative overflow-hidden flex flex-col h-full ${
            isUpcoming ? 'border-zinc-300 hover:border-blue-600 shadow-sm hover:shadow-2xl hover:-translate-y-1' : 'border-zinc-300 opacity-80'
        }`}>
            
            {/* Top Indicator */}
            <div className={`h-2 w-full ${
                isUpcoming ? 'bg-blue-600' : isOngoing ? 'bg-emerald-500' : 'bg-zinc-400'
            }`} />
            ...
            <div className="p-8 flex flex-col h-full">
                ...
                <div className="mt-auto">
                    <div className={`p-4 rounded-2xl flex items-center justify-center gap-3 ${
                        isUpcoming ? 'bg-zinc-50 text-blue-600' : 'bg-zinc-50 text-zinc-400'
                    }`}>
                        {isUpcoming ? (
                            <>
                    ...
                    <button
                        disabled={!isUpcoming}
                        onClick={() => alert(`Redirecting to Candidacy Form for ${election.state}`)}
                        className={`mt-6 w-full py-4 rounded-2xl text-xs font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all ${
                            isUpcoming 
                    ...
                    >
                        {isUpcoming ? (
                            <>Initialize Application <SendHorizontal size={16} /></>
                        ) : (
                            "Ineligible to Apply"
                        )}
                    </button>
